...
export default class Cl_sDatabase {
    ...
    private static conectado: boolean = false;
    private static config: any = null;

    static async conectar(config?: any): Promise<boolean> {
        if (this.conectado && this.pool) return true;
    ...
    private static async crearTablas(): Promise<void> {
        try {
            // Tabla de artículos
    ...
    private static async insertarDatosIniciales(): Promise<void> {
        try {
    ...
    static async ejecutarQuery(query: string, params: any[] = []): Promise<any> {
        if (!this.conectado) {
            await this.conectar();
        }
        if (!this.pool) {
    ...
    static async obtenerArticulos(): Promise<any[]> {
        return await this.ejecutarQuery(
            'SELECT * FROM articulos WHERE activo = TRUE ORDER BY nombre'
        );
    }

    static async obtenerDepartamentos(): Promise<any[]> {
        return await this.ejecutarQuery(
            'SELECT * FROM departamentos WHERE activo = TRUE ORDER BY nombre'
        );
    }

    static async guardarArticulo(articulo: any): Promise<any> {
        const query = `
    ...
    static async guardarEntrada(entrada: any): Promise<any> {
        const query = `
    ...
    static async guardarRequisicion(requisicion: any): Promise<any> {
        const query = `
    ...
    static async guardarDetalleRequisicion(detalle: any): Promise<any> {
        const query = `
    ...
    static async actualizarStock(articuloId: number, cantidad: number): Promise<void> {
    ...
    static async obtenerRequisicionesPendientes(): Promise<any[]> {
        const query = `
    ...
    static async actualizarEstadoRequisicion(id: number, estado: string): Promise<void> {
        const query = `
    ...
    static async obtenerRequisicionesPorDepartamento(departamentoId: number): Promise<any[]> {
        const query = `
    ...
    static async cerrarConexion(): Promise<void> {
        if (this.pool) {
            await this.pool.end();
            this.conectado = false;
            this.pool = null;
