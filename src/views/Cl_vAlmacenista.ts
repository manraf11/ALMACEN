...
export default class Cl_vAlmacenista implements I_vAlmacenista {
    // Callbacks para el controlador
    private avisarRegistrarArticulo: ((datos: any) => void) | null = null;
    private avisarRegistrarEntrada: ((datos: any) => void) | null = null;
    private avisarAprobarRequisicion: ((id: number) => void) | null = null;
    private avisarDespacharRequisicion: ((id: number, detalles: any[]) => void) | null = null;
    private avisarRechazarRequisicion: ((id: number) => void) | null = null;
    private avisarCargarReporteMensual: ((mes: number, anio: number) => void) | null = null;

    // Referencias a elementos del DOM
    private contenedorArticulos: HTMLElement;
    private contenedorEntradas: HTMLElement;
    private contenedorRequisiciones: HTMLElement;
    private contenedorAlertas: HTMLElement;
    private contenedorDashboard: HTMLElement;
    private contenedorReporte: HTMLElement;

    constructor() {
        this.contenedorArticulos = document.getElementById("tablaArticulos") as HTMLElement;
        this.contenedorEntradas = document.getElementById("tablaEntradas") as HTMLElement;
        this.contenedorRequisiciones = document.getElementById("tablaRequisiciones") as HTMLElement;
        this.contenedorAlertas = document.getElementById("alertasStock") as HTMLElement;
        this.contenedorDashboard = document.getElementById("dashboard") as HTMLElement;
        this.contenedorReporte = document.getElementById("reporteMensualContenido") as HTMLElement;
    ...
    public cuandoRegistrarArticulo(callback: (datos: any) => void): void {
        this.avisarRegistrarArticulo = callback;
    }

    public cuandoRegistrarEntrada(callback: (datos: any) => void): void {
        this.avisarRegistrarEntrada = callback;
    }

    public cuandoAprobarRequisicion(callback: (id: number) => void): void {
        this.avisarAprobarRequisicion = callback;
    }

    public cuandoDespacharRequisicion(callback: (id: number, detalles: any[]) => void): void {
        this.avisarDespacharRequisicion = callback;
    }

    public cuandoRechazarRequisicion(callback: (id: number) => void): void {
        this.avisarRechazarRequisicion = callback;
    }

    public cuandoCargarReporteMensual(callback: (mes: number, anio: number) => void): void {
        this.avisarCargarReporteMensual = callback;
    }

    // ========== MÉTODOS PARA MOSTRAR DATOS ==========
    public mostrarArticulos(articulos: Cl_mArticulo[]): void {
        if (!this.contenedorArticulos) return;
    ...
    public mostrarEntradas(entradas: Cl_mEntrada[]): void {
        if (!this.contenedorEntradas) return;
    ...
    public mostrarRequisiciones(requisiciones: Cl_mRequisicion[]): void {
        if (!this.contenedorRequisiciones) return;
    ...
    private mostrarDetalleRequisicion(id: number, requisiciones: Cl_mRequisicion[]): void {
    ...
    private abrirModalDespacho(id: number, requisiciones: Cl_mRequisicion[]): void {
        const req = requisiciones.find(r => r.id === id);
        if (!req) return;
        ...
        if (btnConfirmar) {
            btnConfirmar.addEventListener("click", () => {
                ...
                const detalles: { articuloId: number; cantidad: number }[] = [];

                inputs.forEach((input) => {
                    const index = parseInt(input.getAttribute("data-index") || "0");
                    const cantidad = parseInt((input as HTMLInputElement).value) || 0;
                    if (cantidad > 0) {
    ...
    public mostrarAlertas(alertas: { tipo: 'minimo' | 'maximo'; mensaje: string }[]): void {
    ...
    public mostrarDashboard(datos: {
        totalArticulos: number;
        entradasMes: number;
        salidasMes: number;
    }): void {
        if (!this.contenedorDashboard) return;
    ...
    public mostrarReporteMensual(reporte: any): void {
        if (!this.contenedorReporte) return;
    ...
    private abrirModalArticulo(): void {
    ...
    private abrirModalEntrada(): void {
        this.limpiarFormularios();
    ...
    private cargarArticulosEnSelect(idSelect: string): void {
    ...
    private guardarArticulo(): void {
        const nombre = (document.getElementById("artNombre") as HTMLInputElement).value.trim();
        const descripcion = (document.getElementById("artDescripcion") as HTMLTextAreaElement).value.trim();
        const unidadMedida = (document.getElementById("artUnidad") as HTMLSelectElement).value;
        const stockMinimo = parseInt((document.getElementById("artMinimo") as HTMLInputElement).value) || 5;
        const stockMaximo = parseInt((document.getElementById("artMaximo") as HTMLInputElement).value) || 100;
    ...
    private guardarEntrada(): void {
        const codigoArticulo = (document.getElementById("entArticulo") as HTMLSelectElement).value;
        const cantidad = parseInt((document.getElementById("entCantidad") as HTMLInputElement).value);
        const numeroFactura = (document.getElementById("entFactura") as HTMLInputElement).value.trim();
        const proveedor = (document.getElementById("entProveedor") as HTMLInputElement).value.trim();
        const observaciones = (document.getElementById("entObservaciones") as HTMLTextAreaElement).value.trim();
