...
export default class Cl_vDepartamento implements I_vDepartamento {
    private avisarEnviarRequisicion: ((datos: any) => void) | null = null;
    private avisarVerHistorial: ((departamentoId: number) => void) | null = null;

    private detalles: { articuloId: number; cantidad: number }[] = [];
    private contenedorDetalles: HTMLElement;
    private contenedorHistorial: HTMLElement;

    constructor() {
        this.contenedorDetalles = document.getElementById("detallesRequisicion") as HTMLElement;
        this.contenedorHistorial = document.getElementById("historialRequisiciones") as HTMLElement;
    ...
    private inicializarEventos(): void {
        ...
        if (btnHistorial) {
            btnHistorial.onclick = () => {
                const deptId = parseInt((document.getElementById("reqDepartamentoId") as HTMLInputElement).value);
    ...
    public cuandoEnviarRequisicion(callback: (datos: any) => void): void {
        this.avisarEnviarRequisicion = callback;
    }

    public cuandoVerHistorial(callback: (departamentoId: number) => void): void {
        this.avisarVerHistorial = callback;
    }

    // ========== MÉTODOS PARA MOSTRAR ==========
    public mostrarDepartamento(departamento: Cl_mDepartamento): void {
        const input = document.getElementById("reqDepartamento") as HTMLInputElement;
        if (input) {
            input.value = departamento.getNombreCompleto();
        }

        const inputId = document.getElementById("reqDepartamentoId") as HTMLInputElement;
        if (inputId) {
            inputId.value = String(departamento.id);
        }
    }

    public mostrarArticulosDisponibles(articulos: Cl_mArticulo[]): void {
        const select = document.getElementById("reqArticulo") as HTMLSelectElement;
        if (!select) return;
    ...
    public mostrarHistorial(requisiciones: Cl_mRequisicion[]): void {
        if (!this.contenedorHistorial) return;
    ...
    public mostrarEstadoRequisicion(estado: {
    ...
    public mostrarDetalleRequisicion(requisicion: Cl_mRequisicion): void {
    ...
    public limpiarFormulario(): void {
        ...
        const observaciones = document.getElementById("reqObservaciones") as HTMLTextAreaElement;
        if (observaciones) observaciones.value = '';

        const select = document.getElementById("reqArticulo") as HTMLSelectElement;
        if (select) select.selectedIndex = 0;

        const cantidad = document.getElementById("reqCantidad") as HTMLInputElement;
    ...
    private agregarDetalle(): void {
        const select = document.getElementById("reqArticulo") as HTMLSelectElement;
        const cantidad = parseInt((document.getElementById("reqCantidad") as HTMLInputElement).value);
        ...
        this.actualizarTablaDetalles();
        (document.getElementById("reqCantidad") as HTMLInputElement).value = '';
        select.focus();
    }

    private actualizarTablaDetalles(): void {
        if (!this.contenedorDetalles) return;
    ...
    private enviarRequisicion(): void {
        if (this.detalles.length === 0) {
        ...
        const tipo = (document.getElementById("reqTipo") as HTMLSelectElement).value as 'ORDINARIA' | 'EXTRAORDINARIA';
        ...
        const observaciones = (document.getElementById("reqObservaciones") as HTMLTextAreaElement).value.trim();
        const departamentoId = parseInt((document.getElementById("reqDepartamentoId") as HTMLInputElement).value);
        ...
        this.detalles = [];
        this.actualizarTablaDetalles();
        (document.getElementById("reqObservaciones") as HTMLTextAreaElement).value = '';
    }
